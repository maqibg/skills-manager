...
### Release Overview
- Merges upstream 1.39.0 while keeping this fork's skill notes and repository filtering, and narrows releases to Windows only.
...
- **Windows-only releases** — The release workflow builds and publishes Windows x64 only; this fork no longer produces macOS or Linux builds.

### Developer & Governance
- Notes live in the `skill_notes` table, migrated as schema v8 → v9: upstream's own v8 step is the orphaned-preference cleanup, so both steps are kept and a database from either lineage upgrades in place.
- The fork keeps its independent Tauri updater signing identity, the `maqibg/skills-manager` release endpoint, and retrying a release from an existing tag.
- Merged upstream 1.39.0 (115 commits): reworked bulk operations across the library and workspaces, GitLab Duo as a built-in agent, per-directory skill fetching, and a bounded Git repo cache.
