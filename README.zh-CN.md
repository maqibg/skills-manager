...
- **应用内更新** — 有新版本时应用会主动提醒，并在 Windows 上直接完成安装。不会自行下载或安装：检查只负责告知，安装和重启各需一次点击。

## 安装

### Windows

从 [最新 Release](https://github.com/maqibg/skills-manager/releases/latest) 下载安装包：`.exe` 或 `.msi`。

本 fork 只发布 Windows x64 构建；macOS 与 Linux 构建仍由[上游](https://github.com/xingkongliang/skills-manager)提供。
...
正式 Release 也会提供 Windows x64 的独立 CLI 文件。下载对应的 `skills-manager-cli-*` 放入 PATH 即可。
...
**本 fork 不构建 macOS 版本。** 本 fork 只发布 Windows x64，macOS 安装包请从[上游](https://github.com/xingkongliang/skills-manager)获取。
