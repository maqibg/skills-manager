...
### 发布概览
- 新增单技能备注与 GitHub 仓库筛选，让大型技能库更便于浏览、搜索和整理。

### 用户可见更新
- **可直接在技能卡片上编辑备注** — 检查按钮左侧新增 Pencil 操作，可打开多行编辑器为单个技能记录最多 500 字的备注，并支持修改或清空。
- **可按 GitHub 仓库筛选技能库** — 新增可搜索的单选筛选器，并将等价的 Git、SSH 和 `skills.sh` 来源统一为 `owner/repo`。
- **备注在各工作区统一显示和搜索** — 技能库、全局工作区和项目工作区优先显示用户备注而非原始描述，搜索也会匹配备注内容。

### 开发者与治理更新
- 备注保存在 SQLite v8 数据库和每个技能的 `.skills-manager/skills/<id>.json` 元数据中，Git 备份、恢复和合并流程都会保留备注。
- 此 fork 改用独立的 Tauri updater 签名身份和 `maqibg/skills-manager` 发布地址；发布 workflow 同时支持首个 tag、显式 tag 重试，并完整暂存发布脚本生成的文件。
